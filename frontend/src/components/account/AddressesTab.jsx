  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {addresses.map((addr) => (
          <div key={addr.id} className={`border rounded-lg p-4 ${addr.isDefault ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200'}`}>
            <div className="flex justify-between items-start mb-2">
              <h4 className="font-bold">{addr.title}</h4>
              {addr.isDefault && <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded">پیش‌فرض</span>}
            </div>
            ...
            <div className="flex gap-3 mt-4">
              <button className="text-sm text-emerald-600 hover:underline"><i className="bi bi-pencil"></i> ویرایش</button>
              <button className="text-sm text-red-600 hover:underline"><i className="bi bi-trash"></i> حذف</button>
              {!addr.isDefault && <button className="text-sm text-emerald-600 hover:underline">قرار دادن به عنوان پیش‌فرض</button>}
            </div>
          </div>
        ))}
      </div>
