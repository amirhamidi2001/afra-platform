  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-emerald-700 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cards.map((card) => (
          <div key={card.id} className={`border rounded-lg p-4 ${card.isDefault ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200'}`}>
            <div className="flex justify-between items-start mb-3">
              <i className="bi bi-credit-card text-2xl text-gray-600"></i>
              <div className="flex gap-2">
                {card.isDefault && <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded">پیش‌فرض</span>}
            ...
            <div className="flex gap-2">
              <button className="text-sm text-gray-600 hover:text-emerald-600"><i className="bi bi-pencil"></i> ویرایش</button>
              <button className="text-sm text-red-600 hover:text-red-800"><i className="bi bi-trash"></i> حذف</button>
              {!card.isDefault && <button className="text-sm text-emerald-600 hover:text-emerald-800">قرار دادن به عنوان پیش‌فرض</button>}
            </div>
          </div>
        ))}
      </div>
